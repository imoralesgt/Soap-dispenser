...
#ifndef SERVO_360 //IRM Is this a constrained servo?
  #define SERVO_MIN 0   //IRM Initial servo degrees
  #define SERVO_MAX 45  //IRM Max servo rotation degrees
#else            //IRM Is this a continuous rotation servo?
  #define SERVO_RIGHT 0   //IRM Turn right at full speed
  #define SERVO_STOP  90  //IRM Servo should remain still
  #define SERVO_LEFT  180 //IRM Turn right at full speed
#endif

#ifndef SERVO_360
  #define DELAY_SERVO     350 //IRM pause between servo movements (SERVO_MIN, SERVO_MAX)
#else
  #define LEFT_TO_RIGH_RATIO (float)(390.0/400.0)
  #define DELAY_SERVO_360_LEFT  360
  #define DELAY_SERVO_360_RIGHT (unsigned int)(DELAY_SERVO_360_LEFT*LEFT_TO_RIGH_RATIO)
#endif    
...
void ioSetup(void) {
  ...
  #ifndef SERVO_360 //IRM if this is not a continuous rotation servo
    servoMotor.write(SERVO_MIN);
  #else //IRM otherwise, set the servo to remain still
...
void pourSoap(void) {
  ...
  delay(DELAY_SERVO_360_LEFT);
  servoMotor.write(SERVO_STOP);
  delay(10);
  servoMotor.write(SERVO_RIGHT);
  delay(DELAY_SERVO_360_RIGHT);
  servoMotor.write(SERVO_STOP);
  delay(10);
  servoMotor.write(SERVO_STOP);
#endif
